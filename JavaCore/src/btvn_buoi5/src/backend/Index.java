package btvn_buoi5.src.backend;

import btvn_buoi5.src.entity.Account;
import btvn_buoi5.src.entity.Department;
import btvn_buoi5.src.entity.Position;
...
//        boolean check = accounts.removeAll(accDels);
        ...
        boolean check = accounts.removeIf(account -> account.getFullName().equals(fullname));
        if(check){
    ...
    public Department chooseDepartment(){
        ...
        if (departments.isEmpty()){ // Chưa có thì thêm
            System.out.println("Chưa có department nào nên cần phải thêm trước !!!");
            addDepartment();
            return chooseDepartment();
        }
        else if(departments.size() == 1){ // Nếu chỉ có 1 department thì gán luôn
    ...
    public Position choosePosition(){
        if (positions.isEmpty()){ // Chưa có thì thêm
            System.out.println("Chưa có positions nào nên cần phải thêm trước !!!");
            addPositon();
